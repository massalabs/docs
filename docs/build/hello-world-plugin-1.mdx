...
- **Pre requisites**: We will install the necessary tools to develop the plugin.
- **Part 1**: Setting up the project and create the plugin's API.
- **Part 2**: Deploy to Massa Station.
...
If you are able to produce a similar output in your terminal, then you should be good to go:
...
#### Step 2: Define the server stub configuration
...
#### Step 3: Generate the server stub
...
#### Step 4: Create the logic of the API
...
The `setupGlobalMiddleware` function sets up the cors. The cors will allow the plugin to be called from the Massa Station server.
:::

Once you have crated the `my-plugin.go` file, and copied the code, we are going to install the imported packages. To do so, run the following command in the root directory `my-plugin`:
```bash
go mod tidy
```
...
## Deploy to Massa Station

#### Step 1: Install the Massa Station Plugin package
In order to be able to register the plugin with Massa Station, we need to install the Massa Station Plugin package. Indeed, this package contains the `RegisterPlugin` function that allows us to register the plugin to Massa Station.
...
```
They should be added right after the `server.ConfigureAPI()` line. In the end, your main function should look like this:
...
#### Step 2: Build the plugin binary and move it to the Massa Station plugin folder
...
#### Step 3: Restart Massa Station and access the module page
...
Update the generated code: `go generate ./...`

## Add a handler

Add a handler for the new endpoint in the `api/endpoints.go` file:
...
Open `my-plugin.go`. In the `initializeAPI` function, register the handler to the endpoint:
...
Reinstall the plugin and restart Massa Station. You can follow the same steps as in [Part 1](/docs/tutorial/massa-station-plugin/create-basic-plugin.mdx).

Run the following command to test the `hello` endpoint we implemented:

```bash
curl -X PUT "https://station.massa/plugin/my-name/my-plugin/api/hello?name=John" -H  "accept: application/json"
```
...
The next step is to [add a frontend](/docs/tutorial/massa-station-plugin/create-plugin-with-a-frontend.mdx) to our plugin.
