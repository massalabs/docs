...
Smart contract operations are accessible under the smart contracts sub-client which is accessible via the `smartContracts()` method on the client.
Smart Contracts Client object enables smart contract deployment, read and write operations.
`SmartContractsClient` documentation can be found [here](https://web3.docs.massa.net/classes/SmartContractsClient.html).

### Deployment

The `deploySmartContract` method allows to deploy a smart contract using the WASM binary. This method
...
If no executor of the operation is provided, [base account](https://web3.docs.massa.net/classes/WalletClient.html#setBaseAccount) 
of the wallet will be used.
...
Depending on the smart contract function implementation, the retrieved data might need to be converted into the expected data type. You can find more information about 
Massa Units in the [Utils](/docs/massa-web3-massa-utils) section.


### Write operations

State-changing smart contract operations can be executed via the `callSmartContract` method. It executes a smart contract call and returns the operation id associated.
