...
### Fetching
Events emitted by smart contracts can be directly fetched via the `getFilteredScOutputEvents` method, which takes a
...
### Polling
`massa-web3` provides two methods to poll events. Both methods require a filter, a web3 client and a poll interval.
...
Alternatively, one can make direct use of callback functions as function arguments to execute code in response of event data received or generated errors:
