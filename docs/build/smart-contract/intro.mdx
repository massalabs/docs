...
Smart contracts are programs that interact with the blockchain, accessing ledger storage and modifying the current state of the ledger. They enable developers to build decentralized applications (dApps) that enforce contractual terms autonomously, embodying the principle that **code is law**.

## How smartcontract works on Massa?
...
- According to the amount of gas needed to execute the smart contract, the caller must pay a fee in MAS for the operation to be included in a block by the Massa validators. 
- If a smart contract requires [storage](https://docs.massa.net/docs/learn/storage-costs), it incurs an additional cost.

Finally, smart contracts on Massa are deterministic. Once deployed, their code and interface are publicly available and unchangeable.
