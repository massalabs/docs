...
Inside the contract, you can retrieve the amount of MAS tokens sent with the call by using the `transferredCoins()` function. This returns the amount of MAS in u64 format.
...
1. Getting the Contract's Own Balance:
...
Note that the contract must have sufficient balance to cover the transfer.
