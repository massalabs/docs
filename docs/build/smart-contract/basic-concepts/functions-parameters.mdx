...
Currently, the only supported data type for function arguments and return values is raw bytes, represented as `StaticArray<u8>`. This approach allows for flexible data handling, as any data type can be serialized to raw bytes for transmission and processing.


### Example of a public callable function

Below is an example of a public callable function named hello, which takes a `StaticArray<u8>` as input and returns a `StaticArray<u8>`:
