...
```bash
mkdir station-massa-hello-world && cd station-massa-hello-world
go mod init github.com/my-username/station-massa-hello-world
```

Now, you need a logo.

- Run `mkdir -p web/content && nano web/content/hello_world.svg` to create the file.
...
- Run `nano manifest.json` to create and open the file.
...
- Run `nano Taskfile.yml` to create and open the file.
...
- Define an API for the plugin that will allow MassaStation to interact with it. You can use frameworks like Swagger or Gin to facilitate API development, or choose other suitable frameworks as per your preference.
  - set up the api folder: `mkdir -p api/server`
