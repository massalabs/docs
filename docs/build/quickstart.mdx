...
:::caution
You can find the compatible version of the Massa tools [right here](/docu-dev/docs/build/sdk).
:::
...
Your application should now be running at http://localhost:3000. Here, you'll see the current greeting set in your smart contract. 

🎉Congratulations on building your first dApp!

Remember, Rome wasn't built in a day, and neither are amazing dApps! Keep learning!

## Testing Your Smart Contract

In the development of any software, testing is an essential step. 
It helps you ensure that your code behaves as expected and that it's free of bugs.

At Massa, we use a library called as-pect for unit testing. This library is designed specifically for AssemblyScript and includes a wide range of features such as assertion testing, performance testing, and more.

The following test case ensures that the constructor function of our smart contract does not throw any errors when it is invoked without arguments. You should include all your tests in the **assembly/__tests__** directory of your project.
...
// In this case, we're grouping tests related to the `constructor` function.
describe('constructor function', () => {
  ...
  it('should not throw with no arguments', () => {
    ...
    // Specifically, we're expecting that calling `constructor` with serialized NoArg will not throw an error.
    expect(
...
If your tests pass, you'll get a green checkmark for each one. If they fail, you'll get a red X along with a detailed error message.

:::info
This is a simple example, but you can create more complex tests depending on your smart contract's functionality. For more information about as-pect and its capabilities, refer to the [as-pect documentation](https://as-pect.gitbook.io/as-pect/).
:::
...
- Review more detailed guides on [smart contract development](/docu-dev/docs/build/smart-contract/intro).
- Check out more complex examples of dApps built on Massa.
- Dive deeper into the Massa blockchain documentation and [SDKs](/docu-dev/docs/build/sdk).
