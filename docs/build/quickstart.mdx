...
Great! Let's proceed with writing a basic smart contract. Given we're using AssemblyScript for the Massa blockchain, we'll continue with the "Hello, World!" example. This smart contract will allow users to set and get a greeting message.

Let's create this in a file called **`helloWorld.ts`** in your **`assembly/contracts`** directory:
...
/**
 * @param _args - The argument contains the greeting string
 *
 * @returns empty array
 */
export function setGreeting(_args: StaticArray<u8>): StaticArray<u8> {
  if (!callerHasWriteAccess()) {
    throw new Error("Caller has no write access");
  }
  ...
  generateEvent(`Greeting has been set ${greeting}`);
