...
```
:::note
You might be prompted to install some dependencies. If so, type `y` and press Enter.
:::

With this command, you have created a new folder named massa-hello-world with all the necessary files for developing a Massa smart contract. Navigate to this folder with the following command:
...
Copy the content of the `.env.example` to a newly created `.env` file at the root of your project directory with the following command: 
```bash
cp .env.example .env
```
...
Here we are importing the necessary functions from the Massa AssemblyScript SDK. The Massa AssemblyScript software development kit (SDK) is a collection of functions and objects that wrap the ABIs exposed by the node. In other words it allows your smart contract to interact with the node from within it will be executed.

:::info
If you are familiar with Solidity smart contracts, you will soon notice that some object provided by Massa AssemblyScript SDK are very similar to the ones provided by the Solidity SDK. For exemple, the `Context` object is very similar to the `msg` object in Solidity. It exposes methods such as `Context.transferredCoins()` which would translate to `msg.value` in Solidity. `Context.caller()` would return the address of the function caller like `msg.sender` in solidity. Similarly, the `generateEvent` function is very similar to the `emit` function in Solidity, etc.
...
:::info
In Massa, if you want some data to be persistant on the blockchain, you need to store it in the contract storage. If you are familair with web development, you can think of the storage as the database of your application. The contract storage is a key-value store that can be used to store simple objects such as our "Hello, world!" string, or more complex objects such as serializable object arrays. Then the data can be retrieved from the storage with methods such has `Storage.get()` or `Storage.has()` provided by the SDK. You can learn more about the contract storage [here](docs/build/smart-contract/as-sdk.md).
:::
...
generateEvent(`Greeting has been set`);
```
Here, we are emitting an event to notify that the greeting message has been set. Events are a great way to notify the front ends of a dApp that something has happened. They are similar to events in Solidity. You can learn more about events [here](docs/build/smart-contract/as-sdk.md).
...
This `App.tsx` file serves as the main entry point for your application. We will replace its content with our component's code.
...
```
In order to interact with the blockchain, our front end application is going to need a client. A client is a piece of software that can interact with a service; in our case the service is the Massa blockchain. Thus in the code above, we are initializing a client object. Then, we will use this client's methods to interact with the blockchain and more specifically with our smart contract. 
...
```
In the code above, we are retrieving the "Hellow, world!" greeting message by directly reading the smart contract's storage. 

:::note
Each smart contract has its own storage that can be used to store data. It is also a key-value store.
:::
