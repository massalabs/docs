...
In this section, we will guide you on how to set up and code the frontend for the "Hello World" dApp we are creating. 
We will be using React with TypeScript for building the user interface that interacts with our deployed smart contract on the Massa blockchain. 
Let's get started!

### Setting up your project

Follow these steps to set up your frontend project:

- **Step 1: Initialize Your React Application**

Open your terminal, navigate to your desired directory, and run the following command to create a new React project:

```bash
npx create-react-app my-app --template typescript
```
In the command above, replace my-app with the name you want to give to your project. The `--template typescript` flag is used to set up a TypeScript template for our React application.

Once the installation process is completed, navigate to your project folder:
...
After setting up our project, we need to install the Massa Web3 package. 
This package will allow our frontend to interact with the Massa blockchain. Run the following command to install it:

```bash
npm install @massalabs/massa-web3@1.17.0
```
The version 1.17.0 is the latest version compatible with the Buildnet, Massa's blockchain network for developers.

Now we are ready to start coding the frontend of our dApp!

- **Step 3: Building the Frontend**

Before we start writing our component, navigate to your **src** folder and find the `App.tsx` file. 
This file is the main entry point for your application. We will replace its content with our component's code.
...
function Content() {
  ...
  async function funcGetGreeting() {
    if (web3client) {
      let res = await web3client.publicApi().getDatastoreEntries([{ address: sc_addr, key: strToBytes('greeting') }]);
...
Don't forget to replace `SECRET_KEY` and `SC_ADDRESS` with your own secret key and smart contract address.

### Breaking Down the Code

This code might look complicated, but let's break it down into parts to understand it better.
...
The user interface consists of a centered text display that shows the current greeting that is retrieved from your smart contract's storage.
...
You should be able to see the current greeting that is set in your smart contract.
