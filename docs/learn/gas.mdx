...
* the execution of all the operations of a block: `MAX_BLOCK_GAS = 1_000_000_000`
...
The gas usage of an operation can be estimated by its sender by executing the operation in read-only mode.
...
When a message is dropped, its `coins` are reimbured to the sender but not its `fee`.
