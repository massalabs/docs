...
To determine the amount of coins needed to lock for each byte of storage, the value of 0.000025 Massa coin (MAS) has been chosen.
This value ensures that if half of the total coins (250,000,000 MAS) are locked, the storage limit of 10TB will be reached.

To avoid confusion, a fixed size of 8 bytes has been chosen for each balance.
Therefore, the initial ledger entry for address and balance incurs a cost of (8 + 32) * 0.000025 = 0.001 MAS.
...
$$
Cost_{massa} = 0.000025 * (AddressSize + BalanceSize + BytecodeSize + \sum_i (DataStoreKey_i + DatastoreValue_i))
$$
...
## Examples
When creating an address on the blockchain, a minimum of 0.001 MAS (0.000025 * (32 + 8)) needs to be sent alongside the operation that generates the address (e.g., a transfer).
...
This operation incurs a storage cost of at least 0.001 MAS, calculated as 0.000025 * (10 + 30).
...
However, until a transfer of at least 0.001 Massa is received to your wallet address, it will not appear in any of blockchain's ledgers.
...
2. If a transfer of 10 MAS is received, the balance of the newly created address will show 9.999 MAS, as 0.001 MAS represent the storage cost.
:::

In case you wish to delete the entry in your datastore, you will be refunded the storage costs (0.001 MAS).
