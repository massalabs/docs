...
$$
Cost_{massa} = 0.0001 * (AddressSize + BalanceSize + BytecodeSize + \sum_i (DataStoreKey_i + DatastoreValue_i))
$$
...
## Examples
When creating an address on the blockchain, a minimum of 0.001 MAS needs to be sent alongside the operation that generates the address (e.g., a transfer).
...
This operation incurs a storage cost of at least 0.004 MAS, calculated as 0.0001 * (10 + 30).
