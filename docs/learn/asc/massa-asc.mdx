...
but this time with a full guarantee that the not only the scheduled execution will happen, but also that it will happen exactly at the slot decided by the smart contract.
...
* step 1: the smart contract requests a quote to schedule the call at a given future slot and gives the max amount of gas necessary for the call.
...
* tha target slot at which the call should happen
...
* A fee is automatically calculated and returned by the system using an algorithm inspired by airplane transport pricing:
    ...
    * there is an extra fee for storing the arguments of the call in the state according to the logic given by [storage costs](/docs/learn/storage-costs).
...
* then, any async messages that are ready to execute (if any) are executed as long as there is enough remaining slot gas to fill `MAX_SLOT_GAS + MAX_ASYNC_GAS`.
...
On cancellation, the `coins` of the call are sent back to the smart contract that registered the deferred call, but the `fee` is not refunded.
Also note that, in order to avoid any manipulation of the predictive fee market,
the occupancy of the target slot is not updated when a deferred call is cancelled and remains the same, as if the deferred call was still there.
...
This means that smart contracts can rely on deferred calls to perform actions at an exact time in the future without any risk of failure.
