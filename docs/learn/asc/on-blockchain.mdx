...
Autonomous smart contracts works by emitting messages which will schedule the execution.
The execution can be scheduled based on a target slot, or whenever a specific datastore entry has been written to (whether its value has been changed or not).
Those messages are emitted by smart contracts, through operations sent by users or by autonomous operations.
Messages are then stored in an *asynchronous pool*. Contrary to standard operations, the gas required by autonomous smart contracts is paid upfront.
