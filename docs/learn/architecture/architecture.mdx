...
factories via a `gRPC (and JSON RPC?)` protocol.
...
The Protocol/Network Module implements the protocol connecting consensus nodes. This protocol is supported by a binary and optimized 
transport layer and `does not use JSON RPC`.
...
:::note
More about slashing  mechanism [here](todo insert link to the new section on slashing)
:::
...
date in the *expiration_period* field. Still, some potential attacks can occur by trying to flood the pool with high fees 
