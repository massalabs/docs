...
Different types of operations have different payload run failure triggers:
...
* `CallSC`: fails if any of the following happen:
  ...
  * the target smart contract has no function with the given name and expected prototype.
...
* `ExecuteSC`: fails if any of the following happen:
  ...
  * the smart contract execution in the VM exceeds `max_gas`
  * the smart contract execution in the VM raises a runtime exception
  * the smart contract execution spends more than `max_coins` from the sender

The operation execution process in Massa implies some peculiarities:
* the same operation can be included multiple times in the same block or in different blocks but executed either 0 or 1 times in total
...
Operations are guaranteed to be kept in the `EXECUTED_WITH_SUCCESS` or `EXECUTED_WITH_FAILURE` for at least 10 periods (2 minutes, 40 seconds)
to give time to operators to register those states before they are pruned.
