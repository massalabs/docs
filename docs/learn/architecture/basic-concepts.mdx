...
### Operation execution and finality
...
* block producers incapable of simulating the execution of all candidate operations to choose which ones to include

Massa addresses these issues through multiple techniques.


#### Validation at block reception

Blocks contain arbtirary operations chosen by the block producer.
When a block is received by a node, the node rejects the block (and bans its senders) if any of the following happen:
* any operation is not well-formed
...
Operations included in blocks are not necessarily executed when the block is executed.
...
* discard operations that can not be included by this block producer given

Block producers maximize their gains by choosing 
