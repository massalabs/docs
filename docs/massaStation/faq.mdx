...
On Debian, Ubuntu, and macOS, navigate to `/usr/local/share/massastation` and edit the `config_network.yaml` file.
...
After adding these lines, this node will appear in the network selection dropdown list. Selecting it will direct Massa Station and its plugins, such as Massa Wallet, to connect to your specified node.
