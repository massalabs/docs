...
When running a public instance of DeWeb, be aware that exposing everything on-chain might expose illegal content.
As the owner of the public instance, you might be held responsible for the content served.
We strongly recommend using the block/allow list to ensure your DeWeb instance does not serve illegal content,
thereby protecting yourself from potential legal actions.

Please note that Massa Labs and the Massa Foundation are not responsible for what is hosted on-chain.
We do not block anyone or anything from uploading content to the blockchain.
:::
