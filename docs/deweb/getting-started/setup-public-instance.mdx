...
When running a public instance of DeWeb, be aware that hosting on-chain content might expose illegal material.
As the instance owner, you could be held responsible for the served content.
To protect yourself from legal actions, we strongly recommend using the block/allow list to control the content.

Public instances will operate independently and can set their own content policies.
Each provider can use the blocklist/whitelist system to comply with local regulations.

Please note that Massa Labs and the Massa Foundation are not responsible for on-chain content
and do not restrict uploads to the blockchain.
:::
