...
1. Go on [easy uploader](https://dws.massa.network) and connect your wallet.
...
3. Check data in the confirm panel, click on **approve** and sign the operation on your wallet.
    ![approve immutable](images/approve_immutable_easy_upload.png)

:::note
In the dashboard, websites that have been made immutable have a padlock icon.
...
1. Setup the configuration of the cli if not already done. For more info, check [CLI configuration](/docs/deweb/cli/config)
...
:::info
To skip the confirmation prompt, you can add `-y` or `--yes` option.
:::